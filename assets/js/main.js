...
(function () {
  ...
  (function auth() {
    ...
    $("#authForgot")?.addEventListener("click", async () => {
      ...
      if (!email) { setStatus("Enter your email above first, then tap Forgot password.", false); return; }
      if (!db || !db.enabled) { setStatus("Supabase isn't configured yet.", false); return; }
      const res = await db.resetPassword(email);
      setStatus(res.ok ? "Reset link sent — check your email." : ((res.error && res.error.message) || "Something went wrong. Try again."), res.ok);
    });
  ...
  /* ---------- Show/hide toggle for password fields ----------
     Lets someone check what they actually typed before they submit —
     the real password is never stored anywhere it could be "looked up"
     later, so this is the safe version of that. ---------------------- */
